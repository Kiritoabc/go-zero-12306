...
var (
	...
	tUserMail0RowsExpectAutoSet   = strings.Join(stringx.Remove(tUserMail0FieldNames, "`id`"), ",")
...
type (
	tUserMail0Model interface {
		...
		DeleteByMail(ctx context.Context, session sqlx.Session, mail string) error
	}
...
func (m *defaultTUserMail0Model) Insert(ctx context.Context, session sqlx.Session, data *TUserMail0) (sql.Result, error) {
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		if session != nil {
			return session.ExecCtx(ctx, query, data.Username, data.Mail, data.DeletionTime, time.Now(), time.Now(), data.DelFlag)
		}
		return conn.ExecCtx(ctx, query, data.Username, data.Mail, data.DeletionTime, time.Now(), time.Now(), data.DelFlag)
	})
	return ret, err
}
...
func (m *defaultTUserMail0Model) DeleteByMail(ctx context.Context, session sqlx.Session, mail string) error {
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("update %s set `deletion_time` = ? , `del_flag` = '1' where `mail` = ? ans `del_falg` = '0'", m.table)
		if session != nil {
			return session.ExecCtx(ctx, query, time.Now(), mail)
		}
		return conn.ExecCtx(ctx, query, time.Now(), mail)
	})
	return err
}
